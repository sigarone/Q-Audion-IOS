...
**Action:** Always add `.accessibilityAddTraits(.isButton)`, an `.accessibilityLabel()`, and an `.accessibilityHint()` explaining the action to any `Image`, `Rectangle`, or custom view modifier that uses an `onTapGesture` to trigger logic. For composite error boxes, group with `.accessibilityElement(children: .combine)`.## 2026-08-20 - [SwiftUI accessibilityHint anti-pattern]
**Learning:** When adding `.accessibilityHint` to an element with the `.isButton` trait (or other interactive traits), VoiceOver automatically prepends the interaction instruction (e.g., "Double tap to..."). Including the interaction instruction in the hint itself results in redundant speech.
**Action:** The hint should merely describe the result of the action (e.g., "Copia negli appunti" instead of "Tocca due volte per copiare negli appunti").
