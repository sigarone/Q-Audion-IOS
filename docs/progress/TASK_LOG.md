...
- 2026-04-20 | 1.4-b3 | Profile updateProfile aligned with Android UpdateProfileRequest | (pending commit) | DONE | `AccountApi.updateProfile` signature now `(displayName: String?, statusMessage: String?, avatarUrl: String?)` — all optional, JSON-only. Removed broken multipart branch targeting non-existent `POST /profile` endpoint. `avatar_url` is now sendable (previously missing). Both BCrypto + Upstream impls updated. No existing callers — signature change safe. Resolves §3.3.
