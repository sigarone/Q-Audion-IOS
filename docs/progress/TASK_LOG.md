...
- 2026-04-20 | 1.4-b2 | KMS device/publickey + pending envelope aligned with Android | (pending commit) | DONE | `BCryptoKmsClient.registerPublicKey` drops `device_id` (server derives from auth token) and defaults `key_type` to `"x25519"` matching `DevicePublicKeyRequest`. `getPendingKeys` decodes `{keys:[KmsKeyDto]}` with full DTO (`key_name`, `fingerprint`, `status`, `encrypted_package`, `ephemeral_pubkey`, `nonce`, `created_at?`) instead of the bare `[{key_id,encrypted_key,algorithm,created_at}]` cargo-culted shape. New `BCryptoKmsWireFormatTests.swift`. Resolves §3.9 + §3.10. No external callers — signature change safe.
