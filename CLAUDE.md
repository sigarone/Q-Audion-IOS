...
function `KeyMaterialScrubber.scrub(_:)` (`QAudionEngine/.../Diagnostics/KeyMaterialScrubber.swift`,
a hand-written linear byte scanner, no regex) replaces it with `[REDACTED:keybytes]`. It runs at ring
...
export. Patterns: `derived_key` + the rest of the text; `secret`/`slat`/`salt` + a bracketed group;
...
pairs each are not caught. Tests: `KeyMaterialScrubberTests` (engine) and
`python scripts/test_keymaterial_scrub_parity.py` (Python port, same golden vectors
`Diagnostics/Resources/key-material-scrub-vectors.json`); the port is what replays the real blobs.
