...
| `.github/workflows/ios-ui-smoke.yml` | ACTIVE, manual-only (`workflow_dispatch`) — builds QAudionApp for iOS Simulator (no signing) + runs Maestro UI flows from `maestro/*.yaml`. Added 2026-08-06, UNVERIFIED first run (no Mac to dry-run it) — the WireGuardKitGo/PacketTunnel preBuildScript has never been exercised for a Simulator destination before, most likely first failure point | yes if it fails — read the uploaded `build-sim-log`/`maestro-debug` artifacts first |
