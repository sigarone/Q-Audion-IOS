...
/// from the canonical BIP-39 English wordlist (2048 entries) so the strings
/// are byte-compatible with Android + desktop clients that consume the same
/// list. Cross-platform restore stays correct as long as both sides hash
/// `"word1 word2 ..."` exactly.
///
/// Recovery hash format: `hex(SHA-256("word1 word2 ..."))` lowercase
/// per `RecoverySeedViewModel.computeRecoveryHash`.
///
/// W16.A: replaced the 64-word development placeholder with the full
/// BIP-39 English list. Source of truth:
/// https://github.com/bitcoin/bips/blob/master/bip-0039/english.txt
/// (2048 words, lowercase, alphabetically sorted, no diacritics, no
/// duplicates). The original development wordlist would have caused
/// `validate(words:)` to reject any cross-platform mnemonic that contained
/// words past index 64 — i.e. virtually every real recovery flow.
public enum RecoveryMnemonic {

    /// Canonical BIP-39 English wordlist — exactly 2048 entries. DO NOT
    /// edit by hand; if the canonical list ever shifts upstream, regenerate
    /// from the URL above and re-pin a SHA-256 cross-platform vector.
